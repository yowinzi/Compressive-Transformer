...
class CustomConverter(object):
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        if spkid is not None:
            ...
            spkid = torch.from_numpy(spkid).to(device)
            return xs_pad, ilens, ys_pad, spkid
    ...
    if args.opt == "adadelta":
        if args.criterion == "acc" and mtl_mode != "ctc":
            ...
            trainer.extend(
                ...
                trigger=CompareValueTrigger(
                    "validation/main/loss",                     #"validation/main/loss"
                    lambda best_value, current_value: best_value < current_value,
                ),
            )
            trainer.extend(
                adadelta_eps_decay(args.eps_decay),
                trigger=CompareValueTrigger(
                    "validation/main/loss",                      #"validation/main/loss_rnnt"
                    lambda best_value, current_value: best_value < current_value,
                ),
            )
        ...
        with torch.no_grad():
            for idx, name in enumerate(js.keys(), 1):
                ...
                if args.streaming_mode == "window" and args.num_encs == 1:
                    ...
                    # debug plot
                    # if(name=="BAC009S0764W0174" or name=="sp0.9-BAC009S0002W0138"):
                    #     model.plot_predict(feat,[js[name]['input'][0]['shape'][0]],js[name]['output'][0]['tokenid'].split(),
                    #                     js[name]['output'][0]['token'].split(),name)
