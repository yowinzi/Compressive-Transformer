...
from espnet.nets.pytorch_backend.compressive_transducer.transformer_decoder_layer import (
...
class Decoder(torch.nn.Module):
    ...
    def forward(self, tgt, tgt_mask, memory, memory_mask, x_mems = None, commems = None):
        ...
        for decoder in self.decoders:
            ...
            if x_mems is not None and commems is not None:
                x_q = decoder.x_q.detach() # use detach no grad
                prev_x = decoder.src_attn.simple_attention(x_q,x_mems,x_mems)
                new_x = decoder.src_attn.simple_attention(x_q,commems,commems)
                now_loss_attn = torch.mean((new_x-prev_x)**2)
                loss_attn = loss_attn + now_loss_attn if loss_attn is not None else now_loss_attn
        if self.normalize_before:
            tgt = self.after_norm(tgt)
        if self.output_layer is not None:
            tgt = self.output_layer(tgt)

        return tgt, loss_attn
    ...
    def forward_one_step(self, tgt, tgt_mask,memory, memory_mask=None):
        """Forward one step.
        ...
        for decoder in self.decoders:
            tgt, tgt_mask, memory, memory_mask = decoder(tgt, tgt_mask, memory, memory_mask, None) #memory_mask will error, use None
    ...
    def recognize(self, h, h_masks,recog_args):
        ...
        for i,hi in enumerate(h):
            h_mask = h_masks[0,i].unsqueeze(0).unsqueeze(0) if h_masks is not None else None
            hyp = self.recognize_each_chunk(hyp, h[i], h_mask)
        return [hyp]
    ...
    def recognize_beam(self, h, h_masks, recog_args, rnnlm=None):
        ...
        for i, hi in enumerate(h):
            ...
            while True:
                ...
                if h_masks is not None:
                    ...
                    h_mask = None
                y, _ = self.forward_one_step(ys, ys_mask, h[i,:,:].unsqueeze(0), h_mask) #new_hyp["cache"]
